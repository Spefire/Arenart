...
public class Joueur_Deplacement_J2: MonoBehaviour {
	...
	void Mouvement (){
		if (!stunned) {
			...
			if (Input.GetButtonDown ("J2_Saut") && nb_saut == 1) {
			...
			} else if (Input.GetButtonDown ("J2_Saut") && nb_saut == 2) {
				if (!Camera_Jeu.paused) {
	...
	void OnCollisionEnter (Collision objetInfo){
		if (objetInfo.gameObject.tag == "Ground") {
			...
			if (Pouv_J2_Perso_05.fracased) {
				...
				Pouv_J2_Perso_05.fracased = false;
			}
		}
		if (objetInfo.gameObject.tag == "Arena") {
			Joueur_Stats_J2.getDamage (100, 0);
		}
		if (objetInfo.gameObject.tag == "Encre") {
			Joueur_Stats_J2.getDamage (0, 15);
			DestroyObject (objetInfo.gameObject);
		}
	}

	void OnTriggerEnter(Collider objetInfo) {
		if (objetInfo.gameObject.tag == "J1") {
			if (Pouv_J1_Perso_05.fracased) {
				double calculDegats = Pouv_J1_Perso_05.Degats * (1.25 - Joueur_Stats_J2.Resistance / 100);
				Joueur_Stats_J2.getDamage (calculDegats, 10);
		...
		if (objetInfo.gameObject.tag == "BallOmbre") {
			double calculDegats = Pouv_J1_Perso_04.Degats * (1.5 - Joueur_Stats_J2.Resistance / 100);
			...
			Physics.IgnoreCollision (instantiatedProjectile.GetComponent<Collider> (), transform.root.GetComponent<Collider> ());
			Pouv_J1_Perso_03.spirited = true;
			Pouv_J1_Perso_03.time_spirit = (double)Time.time;
			stunned = true;
			double calculDegats = Pouv_J1_Perso_03.Degats * (1.5 - Joueur_Stats_J2.Resistance / 100);
			Joueur_Stats_J2.getDamage (calculDegats, 20);
			DestroyObject (objetInfo.gameObject);
		}
		else if (objetInfo.gameObject.tag == "BouleFeu") {
			double calculDegats = Pouv_J1_Perso_01.Degats * (1.5 - Joueur_Stats_J2.Resistance / 100);
			print (calculDegats);
			Joueur_Stats_J2.getDamage (calculDegats, 10);
			DestroyObject (objetInfo.gameObject);
		}
		else if (objetInfo.gameObject.tag == "Meteore") {
			double calculDegats = Pouv_J1_Perso_01.Degats * (2 - Joueur_Stats_J2.Resistance / 100);
			Joueur_Stats_J2.getDamage (calculDegats, 20);
			...
			double calculDegats = Pouv_J1_Perso_02.Degats * (1.5 - Joueur_Stats_J2.Resistance / 100);
			Joueur_Stats_J2.getDamage (calculDegats, 10);
			DestroyObject (objetInfo.gameObject);
		}
		else if (objetInfo.gameObject.tag == "LancesGlace") {
			double calculDegats = Pouv_J1_Perso_02.Degats * (1.25 - Joueur_Stats_J2.Resistance / 100);
			Joueur_Stats_J2.getDamage (calculDegats, 5);
			DestroyObject (objetInfo.gameObject);
		}
		else if (objetInfo.gameObject.tag == "FouetEau") {
			if (Joueur_Deplacement_J1.versGauche && transform.position.x < ennemi.transform.position.x) {
				double calculDegats = Pouv_J1_Perso_02.Degats * (1.5 - Joueur_Stats_J2.Resistance / 100);
				Joueur_Stats_J2.getDamage (calculDegats, 10);
			...
			} else if (!Joueur_Deplacement_J1.versGauche && transform.position.x > ennemi.transform.position.x) {
				double calculDegats = Pouv_J1_Perso_02.Degats * (1.5 - Joueur_Stats_J2.Resistance / 100);
				Joueur_Stats_J2.getDamage (calculDegats, 10);
				GetComponent<Rigidbody>().AddForce (Vector3.up * 50, ForceMode.Impulse);
				GetComponent<Rigidbody>().AddForce (Vector3.right * 50, ForceMode.Impulse);
			...
			if (Joueur_Deplacement_J1.versGauche && transform.position.x < ennemi.transform.position.x) {
				double calculDegats = Pouv_J1_Perso_05.Degats * (1.5 - Joueur_Stats_J2.Resistance / 100);
				Joueur_Stats_J2.getDamage (calculDegats, 10);
				GetComponent<Rigidbody>().AddForce (Vector3.up * 50, ForceMode.Impulse);
				GetComponent<Rigidbody>().AddForce (-Vector3.right * 50, ForceMode.Impulse);
			} else if (!Joueur_Deplacement_J1.versGauche && transform.position.x > ennemi.transform.position.x) {
