...
public class Choice_J2 : MonoBehaviour {
	...
	void Update () {
		if (Camera_Menu.pos == pos) {
			if (choice == 0 && (Game_Inputs.J2_Bas || Game_Inputs.J2_Attaque)) {
				choice = 2;
				render.material.mainTexture = texture_choice_2;
			}
			else if ((choice == 1 || choice == 2) && Game_Inputs.J2_Haut) {
				choice = 0;
				render.material.mainTexture = texture_choice_0;
				aud.PlayOneShot (son_selection);
			}
			else if (choice == 1 && Game_Inputs.J2_Droit) {
				choice = 2;
				render.material.mainTexture = texture_choice_2;
				aud.PlayOneShot (son_selection);
			}
			else if (choice == 2 && Game_Inputs.J2_Gauche) {
				choice = 1;
